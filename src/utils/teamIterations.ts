...
export function ensureTeamIterations(
  ...
  const normalizedIterations = normalizeTeamIterationMarkers((data.teamIterations ?? []).map(iteration => {
    ...
    const normalizedIteration: TeamIteration = {
      ...
      createdAt,
      updatedAt: iteration.updatedAt || createdAt,
      note: iteration.note,
  ...
  legacyIteration.updatedAt = legacyIteration.createdAt;
...
export function updateTeamIterationMetadata(
  ...
  const updatedIterations = existingIterations.map(iteration => {
    ...
    const updatedIteration: TeamIteration = {
      ...iteration,
      updatedAt: now,
    };

    if (updates.name !== undefined) {
      updatedIteration.name = updates.name;
    }

    if (updates.note !== undefined) {
      const trimmedNote = updates.note.trim();
      updatedIteration.note = trimmedNote || undefined;
    }

    if (updates.isPreferred !== undefined) {
      updatedIteration.isPreferred = updates.isPreferred;
    }

    if (updates.isFinal !== undefined) {
      updatedIteration.isFinal = updates.isFinal;
    }
